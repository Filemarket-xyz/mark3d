...
func main() {
	...
	rpcClient, err := rpc.DialContext(ctx, cfg.RpcUri)
	if err != nil {
	...
	if err := loadKeys(ctx, rdb); err != nil {
		log.Fatal(err)
	}
	...
	for {
		select {
		case <-ticker.C:
			if err := autoseller.processTimers(ctx); err != nil {
...
func (a *Autoseller) process() error {
	res, err := http.DefaultClient.Get(a.Cfg.IndexerAddr)
	if err != nil {
		return err
	}
	...
	for _, r := range resp {
		if err := a.approveSingleResponse(context.Background(), r); err != nil {
			log.Printf("failed to process single response: %#v. Error: %v\n", r, err)
			continue
		}
		if err := a.addTimer(context.Background(), r.TokenId); err != nil {
